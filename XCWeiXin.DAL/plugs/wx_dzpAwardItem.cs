...
namespace XCWeiXin.DAL
{
	...
	public partial class wx_dzpAwardItem
	{
		...
		/// 增加一条数据
		/// </summary>
		public int Add(XCWeiXin.Model.wx_dzpAwardItem model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into wx_dzpAwardItem(");
            strSql.Append("actId,jxName,jpName,jpNum,jpRealNum,sort_id,createDate,jiaodu_min,jiaodu_max,jpimage,jpattr)");
            ...
            SqlParameter[] parameters = {
                    ...
                    new SqlParameter("@jpimage", SqlDbType.VarChar,200),
                    new SqlParameter("@jpattr", SqlDbType.Int,4)};
            ...
            parameters[9].Value = model.jpimage;
            parameters[10].Value = model.jpattr;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(XCWeiXin.Model.wx_dzpAwardItem model)
		{
			StringBuilder strSql=new StringBuilder();
            ...
            strSql.Append("jpimage=@jpimage");
            strSql.Append("jpattr=@jpattr");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@actId", SqlDbType.Int,4),
					new SqlParameter("@jxName", SqlDbType.VarChar,50),
		...
		/// 得到一个对象实体
		/// </summary>
		public XCWeiXin.Model.wx_dzpAwardItem GetModel(int id)
		{
			
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select  top 1 id,actId,jxName,jpName,jpNum,jpRealNum,sort_id,createDate,jpattr,jiaodu_min,jiaodu_max from wx_dzpAwardItem ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
			...
			XCWeiXin.Model.wx_dzpAwardItem model=new XCWeiXin.Model.wx_dzpAwardItem();
		...
		/// 得到一个对象实体
		/// </summary>
		public XCWeiXin.Model.wx_dzpAwardItem DataRowToModel(DataRow row)
		{
			XCWeiXin.Model.wx_dzpAwardItem model=new XCWeiXin.Model.wx_dzpAwardItem();
			if (row != null)
			{
                ...
                if (row["jpimage"] != null && row["jpimage"].ToString() != "")
                {
                    model.jpimage = row["jpimage"].ToString();
                }
                if (row["jpattr"] != null && row["jpattr"].ToString() != "")
                {
                    model.jpattr = int.Parse(row["jpattr"].ToString());
                }
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select id,actId,jxName,jpName,jpNum,jpRealNum,sort_id,createDate,jiaodu_min,jiaodu_max,jpimage,jpattr ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
            ...
            strSql.Append(" id,actId,jxName,jpName,jpNum,jpRealNum,sort_id,createDate,jiaodu_min,jiaodu_max,jpattr ");
			strSql.Append(" FROM wx_dzpAwardItem ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
