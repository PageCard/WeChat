...
namespace XCWeiXin.DAL
{
	...
	public partial class wx_vote_item
	{
		...
		public XCWeiXin.Model.wx_vote_item GetModel(int id)
		{
			...
			XCWeiXin.Model.wx_vote_item model=new XCWeiXin.Model.wx_vote_item();
		...
		/// 得到一个对象实体
		/// </summary>
		public XCWeiXin.Model.wx_vote_item DataRowToModel(DataRow row)
		{
			XCWeiXin.Model.wx_vote_item model=new XCWeiXin.Model.wx_vote_item();
			if (row != null)
			{
        ...
        /// 对某一列求和
        /// </summary>
        public int GetFieldSum(string field,string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select sum("+field+") FROM wx_vote_item ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
