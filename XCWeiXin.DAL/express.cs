...
namespace XCWeiXin.DAL
{
    ...
    public partial class express
    {
        ...
        /// 获得查询分页数据
        /// </summary>
        public DataSet GetListshop(int pageSize, int pageIndex,string tablae, string strWhere, string filedOrder, out int recordCount)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM  " + tablae);
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
