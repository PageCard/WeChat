...
namespace XCWeiXin.DAL
{
	...
	public partial class wx_shop_category
	{
		...
		public XCWeiXin.Model.wx_shop_category GetModel(int id)
		{
			...
			XCWeiXin.Model.wx_shop_category model=new XCWeiXin.Model.wx_shop_category();
		...
		/// 得到一个对象实体
		/// </summary>
		public XCWeiXin.Model.wx_shop_category DataRowToModel(DataRow row)
		{
			XCWeiXin.Model.wx_shop_category model=new XCWeiXin.Model.wx_shop_category();
			if (row != null)
			{
        ...
        public DataTable GetWCodeList(int wid, int parent_id,int topNum)
        {
            ...
            strSql.Append(" id,title,code,parent_id,class_list,class_layer,sort_id,link_url,img_url,class_content,remark,seo_title,seo_keywords,seo_description,wid,ico_url from wx_shop_category ");
