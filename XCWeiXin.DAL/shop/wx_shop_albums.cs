...
namespace XCWeiXin.DAL
{
	...
	public partial class wx_shop_albums
	{
		...
		/// 增加一条数据
		/// </summary>
		public int Add(XCWeiXin.Model.wx_shop_albums model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(XCWeiXin.Model.wx_shop_albums model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 得到一个对象实体
		/// </summary>
		public XCWeiXin.Model.wx_shop_albums GetModel(int id)
		{
			...
			XCWeiXin.Model.wx_shop_albums model=new XCWeiXin.Model.wx_shop_albums();
		...
		/// 得到一个对象实体
		/// </summary>
		public XCWeiXin.Model.wx_shop_albums DataRowToModel(DataRow row)
		{
			XCWeiXin.Model.wx_shop_albums model=new XCWeiXin.Model.wx_shop_albums();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select id,productId,thumb_path,original_path,remark,add_time,wid,px ");
	   ...
	   /// <summary>
        ...
        /// </summary>
        public void DeleteshopSetting(List<Model.wx_shop_albums> models, int shopsettingId)
        {
            StringBuilder idList = new StringBuilder();
            if (models != null)
            {
            ...
            string id_list = Utils.DelLastChar(idList.ToString(), ",");
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,thumb_path,original_path from wx_shop_albums where shopsettingId=" + shopsettingId);
            if (!string.IsNullOrEmpty(id_list))
            {
                strSql.Append(" and id not in(" + id_list + ")");
            }
            ...
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                int rows = DbHelperSQL.ExecuteSql("delete from  wx_shop_albums  where id=" + dr["id"].ToString()); //删除数据库
                if (rows > 0)
                {
                    Utils.DeleteFile(dr["thumb_path"].ToString()); //删除缩略图
                    Utils.DeleteFile(dr["original_path"].ToString()); //删除原图
        ...
        /// </summary>
        public void DeleteClass(List<Model.wx_shop_albums> models, int classId)
        {
            StringBuilder idList = new StringBuilder();
            if (models != null)
            {
            ...
            string id_list = Utils.DelLastChar(idList.ToString(), ",");
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,thumb_path,original_path from wx_shop_albums where classId=" + classId);
            if (!string.IsNullOrEmpty(id_list))
            {
                strSql.Append(" and id not in(" + id_list + ")");
            }
