...
namespace XCWeiXin.DAL
{
	...
	public partial class wx_shop_product
	{
		...
		/// 得到一个对象实体
		/// </summary>
		public XCWeiXin.Model.wx_shop_product DataRowToModel(DataRow row)
		{
			XCWeiXin.Model.wx_shop_product model=new XCWeiXin.Model.wx_shop_product();
			if (row != null)
			{
        ...
        public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
            ...
            strSql.Append(" link_url='',(select top 1 thumb_path from dbo.wx_shop_albums where  productId=p.id order by px asc) as productpic,* from wx_shop_product p ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
        ...
        /// 得到一个对象实体
        /// </summary>
        public XCWeiXin.Model.wx_shop_product GetModel(int id)
        {
        ...
        public int Add(Model.wx_shop_product model)
        {
            ...
            if (model.albums != null)
            {
                ...
                foreach (Model.wx_shop_albums modelt in model.albums)
                {
                    ...
                    strSql2.Append("productId,thumb_path,original_path,remark,px)");
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(XCWeiXin.Model.wx_shop_product model)
        {
                        ...
                        if (model.albums != null)
                        {
                            ...
                            foreach (Model.wx_shop_albums modelt in model.albums)
                            {
                                ...
                                else
                                {
                                    ...
                                    strSql3.Append("productId,thumb_path,original_path,remark,px)");
                                    strSql3.Append(" values (");
                                    strSql3.Append("@productId,@thumb_path,@original_path,@remark,@px)");
                                    SqlParameter[] parameters3 = {
					                        new SqlParameter("@productId", SqlDbType.Int,4),
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select  link_url='',(select top 1 thumb_path from dbo.wx_shop_albums where  productId=p.id order by px asc) as productpic,* from wx_shop_product p ");
        ...
        /// 搜索，主要字段有名称，分类，简介
        /// </summary>
        public DataSet SoGetList(int wid, string key, int pageSize, int pageIndex, string strWhere, string filedOrder, out int recordCount)
        {
           
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select p_count='', link_url='',(select top 1 thumb_path from dbo.wx_shop_albums where  productId=p.id order by px asc) as productpic,id,productName,shortDesc,salePrice from wx_shop_product p ");
            strSql.Append(" where  wid=" + wid);
            strSql.Append(" and (productName like '%" + key + "%' or shortDesc  like '%" + key + "%' or  categoryId in(select id from  wx_shop_category where title like '%" + key + "%'))");
