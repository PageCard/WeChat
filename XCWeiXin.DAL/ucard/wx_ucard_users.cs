...
namespace XCWeiXin.DAL
{
	...
	public partial class wx_ucard_users
	{
        ...
        /// 该用户是否存在
        /// </summary>
        public bool getUserExists(string openid)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from wx_ucard_users");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(XCWeiXin.Model.wx_ucard_users model)
		{
			StringBuilder strSql=new StringBuilder();
			...
			SqlParameter[] parameters = {
                    ...
                    new SqlParameter("@language", SqlDbType.VarChar,20),
                    ...
                    new SqlParameter("@headimgul", SqlDbType.VarChar,500),
                    new SqlParameter("@subscribe_time", SqlDbType.DateTime),
                    new SqlParameter("@unsubscribe_time", SqlDbType.DateTime),
            ...
            //2016.4.28
            parameters[23].Value = model.subscribe;
            parameters[24].Value = model.language;
            parameters[25].Value = model.city;
            parameters[26].Value = model.province;
            parameters[27].Value = model.country;
            parameters[28].Value = model.headimgul;
            parameters[29].Value = model.subscribe_time;
            parameters[30].Value = model.unsubscribe_time;
            parameters[31].Value = model.groupid;
            parameters[32].Value = model.groupname;
		...
		/// 得到一个对象实体
		/// </summary>
		public XCWeiXin.Model.wx_ucard_users GetModel(int id)
		{
			
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select  top 1 id,wid,sid,openid,cardNo,pwd,degreeId,sex,birthday,wxName,realName,age,qq,regTime,regIp,telphone,mobile,email,addr,endDate,ttScore,qdScore,consumeScore,consumeMoney,subscribe,language,city,province,country,headimgul,subscribe_time,unsubscribe_time,groupid,groupname from wx_ucard_users ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
			...
			XCWeiXin.Model.wx_ucard_users model=new XCWeiXin.Model.wx_ucard_users();
        ...
        /// openid得到一个对象实体
        /// </summary>
        public XCWeiXin.Model.wx_ucard_users GetModel(string openid)
        {
		...
		/// 得到一个对象实体
		/// </summary>
		public XCWeiXin.Model.wx_ucard_users DataRowToModel(DataRow row)
		{
			XCWeiXin.Model.wx_ucard_users model=new XCWeiXin.Model.wx_ucard_users();
			if (row != null)
			{
                ...
                //2016.4.28
                if (row["unsubscribe_time"] != null && row["unsubscribe_time"].ToString() != "")
                {
                    model.unsubscribe_time = DateTime.Parse(row["unsubscribe_time"].ToString());
                }
                ...
                if (row["headimgul"] != null)
                {
                    model.headimgul = row["headimgul"].ToString();
                }
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select id,wid,sid,openid,cardNo,pwd,degreeId,sex,birthday,wxName,realName,age,qq,regTime,regIp,telphone,mobile,email,addr,endDate,ttScore,qdScore,consumeScore,consumeMoney,subscribe,language,city,province,country,headimgul,subscribe_time,unsubscribe_time,groupid,groupname ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
            ...
            strSql.Append(" id,wid,sid,openid,cardNo,pwd,degreeId,sex,birthday,wxName,realName,age,qq,regTime,regIp,telphone,mobile,email,addr,endDate,ttScore,qdScore,consumeScore,consumeMoney,subscribe,language,city,province,country,headimgul,subscribe_time,unsubscribe_time,groupid,groupname ");
			strSql.Append(" FROM wx_ucard_users ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
        ...
        /// 得到一个对象实体
        /// </summary>
        public XCWeiXin.Model.wx_ucard_users GetStoreUserInfo(string oepnid, int sid)
        {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(XCWeiXin.Model.wx_ucard_users model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into wx_ucard_users(");
            strSql.Append("wid,sid,openid,cardNo,pwd,degreeId,sex,birthday,wxName,realName,age,qq,regTime,regIp,telphone,mobile,email,addr,endDate,ttScore,qdScore,consumeScore,consumeMoney,subscribe,language,city,province,country,headimgul,subscribe_time,unsubscribe_time,groupid,groupname)");
            strSql.Append(" values (");
            strSql.Append("@wid,@sid,@openid,dbo.ufn_ucard_maxCardNo(),@pwd,@degreeId,@sex,@birthday,@wxName,@realName,@age,@qq,@regTime,@regIp,@telphone,@mobile,@email,@addr,@endDate,@ttScore,@qdScore,@consumeScore,@consumeMoney,@subscribe,@language,@city,@province,@country,@headimgul,@subscribe_time,@unsubscribe_time,@groupid,@groupname)");
           ...
           // parameters[3].Value = model.cardNo;
            ...
            //2016.4.28
            parameters[22].Value = model.subscribe;
            ...
            parameters[27].Value = model.headimgul;
            parameters[28].Value = model.subscribe_time;
            parameters[29].Value = model.unsubscribe_time;
            parameters[30].Value = model.groupid;
            parameters[31].Value = model.groupname;
