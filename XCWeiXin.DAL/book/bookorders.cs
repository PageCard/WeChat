...
namespace XCWeiXin.DAL
{
    ...
    public partial class bookorders
    {
        ...
        public bookorders(string _databaseprefix)
        {
        ...
        public Model.bookorders GetModel(int id)
        {
            ...
            Model.bookorders model = new Model.bookorders();
        ...
        {
            IList<Model.bookorders> retlist = new List<Model.bookorders>();
            StringBuilder strSql = new StringBuilder();
            ...
            List<Model.wx_book_product> models = new List<Model.wx_book_product>();
            if (ds.Tables[0].Rows.Count > 0)
            {
                DataRow dr;
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    model = new Model.bookorders();
                    ...
                    #region 子表信息
                    StringBuilder strSql2 = new StringBuilder();
                    strSql2.Append("select  (select top 1 original_path from  wx_book_albums where  productId=p.id) as productpic,p.*,og.quantity,og.real_price from wx_book_product p right join  dt_order_goods og on p.id=og.goods_id where og.order_id=@id");
                    ...
                    if (ds2.Tables[0].Rows.Count > 0)
                    {
                        ...
                        models = new List<Model.wx_book_product>();
                        Model.wx_book_product modelt;
                        for (int n = 0; n < count; n++)
                        {
                            modelt = new Model.wx_book_product();
                            #region 子表，for循环
