...
{
	/// <summary>
	/// 数据访问类:wx_book_category
	/// </summary>
	public partial class wx_book_category
	{
		public wx_book_category()
		{}
		#region  BasicMethod
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("id", "wx_book_category"); 
		}
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from wx_book_category");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from wx_book_category ");
		...
		public DataSet GetList(string strWhere)
		{
			...
			strSql.Append(" FROM wx_book_category ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from wx_book_category T ");
        ...
        {
            //查找父节点信息
            Model.wx_book_category model = GetModel(conn, trans, parent_id);
            if (model != null)
            {
                //查找子节点
                string strSql = "select id from wx_book_category where parent_id=" + parent_id;
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    DbHelperSQL.ExecuteSql(conn, trans, "update wx_book_category set class_list='" + class_list + "', class_layer=" + class_layer + " where id=" + id); //带事务
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Model.wx_book_category model)
        {
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    ...
                    {
                        StringBuilder strSql = new StringBuilder();
                        strSql.Append("insert into wx_book_category(");
                        ...
                        if (model.albums != null)
                        {
                            ...
                            foreach (Model.wx_book_albums modelt in model.albums)
                            {
                                ...
                                strSql2.Append("classId,thumb_path,original_path,remark,add_time,wid)");
                        ...
                        {
                            Model.wx_book_category model2 = GetModel(conn, trans, model.parent_id.Value); //带事务
                        ...
                        //修改节点列表和深度
                        DbHelperSQL.ExecuteSql(conn, trans, "update wx_book_category set class_list='" + model.class_list + "', class_layer=" + model.class_layer + " where id=" + model.id); //带事务
                        trans.Commit();
                    }
                    catch
                    {
        ...
        /// 得到一个对象实体(重载，带事务)
        /// </summary>
        public Model.wx_book_category GetModel(SqlConnection conn, SqlTransaction trans, int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1  id,title,code,parent_id,class_list,class_layer,sort_id,link_url,img_url,class_content,remark,seo_title,seo_keywords,seo_description,wid,ico_url from wx_book_category  ");
            strSql.Append(" where id=@id");
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Model.wx_book_category model)
        {
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                conn.Open();
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            //查找旧数据
                            Model.wx_book_category oldModel = GetModel(model.id);
                            //查找旧父节点数据
                            string class_list = "," + model.parent_id + ",";
                            int class_layer = 1;
                            if (oldModel.parent_id > 0)
                            {
                                Model.wx_book_category oldParentModel = GetModel(conn, trans, oldModel.parent_id.Value); //带事务
                                class_list = oldParentModel.class_list + model.parent_id + ",";
                                class_layer = oldParentModel.class_layer.Value + 1;
                            }
                            //先提升选中的父节点
                            DbHelperSQL.ExecuteSql(conn, trans, "update wx_book_category set parent_id=" + oldModel.parent_id + ",class_list='" + class_list + "', class_layer=" + class_layer + " where id=" + model.parent_id); //带事务
                        ...
                        StringBuilder strSql = new StringBuilder();
                        strSql.Append("update wx_book_category set ");
                        ...
                        new wx_book_albums().DeleteClass(model.albums, model.id);
                        if (model.albums != null)
                        {
                            ...
                            foreach (Model.wx_book_albums modelt in model.albums)
                            {
                                ...
                                if (modelt.id > 0)
                                {
                                    strSql3.Append("update  wx_book_albums set ");
                                ...
                                {
                                    strSql3.Append("insert into wx_book_albums(");
                                    strSql3.Append("classId,thumb_path,original_path,remark,add_time,wid)");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update wx_book_category set " + strValue);
            strSql.Append(" where id=" + id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select   id,title,code,parent_id,class_list,class_layer,sort_id,link_url,img_url,class_content,remark,seo_title,seo_keywords,seo_description,wid,ico_url from wx_book_category ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select  * from wx_book_category ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where  " + strWhere);
            }
        ...
        /// <returns></returns>
        public IList<Model.wx_book_category> GetCategoryListByWid(int wid, int topNum, int parentId, int class_layer)
        {
            IList<Model.wx_book_category> categoryList = new List<Model.wx_book_category>();
            ...
            strSql.Append(" id,title,code,parent_id,class_list,class_layer,sort_id,link_url,img_url,class_content,remark,seo_title,seo_keywords,seo_description,wid,ico_url from wx_book_category   where 1=1 ");
            if (parentId != -1)
            {
            ...
            Model.wx_book_category category = new Model.wx_book_category();
            while (sr.Read())
            {
                category = new Model.wx_book_category();
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public Model.wx_book_category GetCategoryByWid(int wid, int id)
        {

            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1");
            strSql.Append(" id,title,code,parent_id,class_list,class_layer,sort_id,link_url,img_url,class_content,remark,seo_title,seo_keywords,seo_description,wid,ico_url  ");
            strSql.Append(" from wx_book_category  where   id=@id  and wid=@wid");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4),
                    new SqlParameter("@wid", SqlDbType.Int,4)
			};
        ...
        public bool DeleteCategory(int id)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from wx_book_category ");
            strSql.Append(" where id=@id and   id not in(select categoryId  from wx_book_product) ");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
            parameters[0].Value = id;
