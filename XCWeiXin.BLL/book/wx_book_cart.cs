...
namespace XCWeiXin.BLL
{
	...
	{
		private readonly XCWeiXin.DAL.wx_book_cart dal=new XCWeiXin.DAL.wx_book_cart();
		public wx_book_cart()
		{}
		#region  BasicMethod
		...
		/// 获得数据列表
		/// </summary>
		public List<XCWeiXin.Model.wx_book_cart> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<XCWeiXin.Model.wx_book_cart> DataTableToList(DataTable dt)
		{
			List<XCWeiXin.Model.wx_book_cart> modelList = new List<XCWeiXin.Model.wx_book_cart>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				XCWeiXin.Model.wx_book_cart model;
        ...
        public List<Model.cartProduct> GetCartList(string openid, int wid)
        {
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    cproduct.skuId = MyCommFun.ObjToStr(dr["skuId"]);
                    ...
                    cproduct.productUrl = "/book/detail.aspx?wid=" + cproduct.wid + "&pid=" + cproduct.productId + "&openid=" + cproduct.openid;
                    cproduct.seq = i;
