...
namespace XCWeiXin.Web.weixin.dzp
{
    /// <summary>
    /// dzpAct 的摘要说明
    /// </summary>
    public class dzpAct : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (_action == "choujiang")
            {
                ...
                try
                {
                    ...
                    if (dajiang.jpattr == 2)
                    {
                        jsonDict.Add("error", "moretimes");
                        jsonDict.Add("content", "您获得了再抽一次奖的机会！");
                        jsonDict.Add("sortid", dajiang.sort_id.ToString());
                        context.Response.Write(MyCommFun.getJsonStr(jsonDict));
                        return;
                    }
                    else
                    ...
                    if (award != null)
                    {
                        //您中奖了
                        jsonDict.Add("error", "notimes");
                        jsonDict.Add("content", "您已参加过了，欢迎下次再来！");
                        context.Response.Write(MyCommFun.getJsonStr(jsonDict));
                        return;
                    }
                    ...
                    if (radNum < syZjNum)
                    {
                        ...
                        if (dajiang != null)
                        {
                          ...
                          if (dajiang.jpattr == 3)//判断是否为参与奖3
                          {
                              
                              int? gettime = getTempTime(openid, aid);
                              if (gettime <= dzpAction.personMaxTimes)
                              {
                                  jsonDict.Add("error", "nexttimes");
                                  jsonDict.Add("sortid", dajiang.sort_id.ToString());
                                  jsonDict.Add("content", "很遗憾没有中奖，您还有" + (dzpAction.personMaxTimes - gettime) + "次机会！");
                              }
                              else
                              {
                                  jsonDict.Add("error", "notimes");
                                  jsonDict.Add("content", "很遗憾，您没有中奖，欢迎下次再来！");
                              }
                                  context.Response.Write(MyCommFun.getJsonStr(jsonDict));                              
                              return;
                          }
                          else if (dajiang.jpattr == 1)
                          {
                              ...
                              jsonDict.Add("jxname", dajiang.jxName.Replace("p", ""));
        ...
        private Model.wx_dzpAwardItem getZJItem(IList<Model.wx_dzpAwardItem> itemlist, IList<Model.wx_dzpAwardUser> haszjlist)
        {
            ...
            for (int i = 0; i < itemlist.Count; i++)
            {
                ...
                for (int j = 0; j < tmpSYNum; j++)
                {
                    ...
                    stmpItem.jxName = tmpItem.jxName.Replace("p","");
                    stmpItem.sort_id = tmpItem.sort_id;
                    stmpItem.jpattr = tmpItem.jpattr;
        ...
        private int? getTempTime(string openid, int aid)
        {
        ...
        public string Get_snumber(int aid)
        {
            ...
            return "HKWYX" + aid + "_" + MyCommFun.ConvertDateTimeInt(DateTime.Now) + radNum;
        }
