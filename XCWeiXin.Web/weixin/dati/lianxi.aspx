<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="lianxi.aspx.cs" Inherits="XCWeiXin.Web.weixin.dati.lianxi" %>
<!DOCTYPE html>
...
<div class="headimg"><img src="<%=headimg%>" /></div>
 ...
 <%if(!isusersub){%> 
   ...
   <a href="#step" id="bntnext" class="button button-big button-fill button-danger" onclick="timer(parseInt(<%=dtime %>*60))">开始练习答题</a>      
   </div>
   <%} %>  
    </div>
    </div>
...
<div class="page" id='step'>
  ...
  <%for (int i = 0; i < dxDT.Tables[0].Rows.Count; i++)
    ...
    <input type="hidden" id="re<%=i %>an" value="<%=dxDT.Tables[0].Rows[i]["answer"]%>" />
      <div class="card-content-inner" style="color:#fff"><%=i+1 %>. <%=dxDT.Tables[0].Rows[i]["title"]%>　<span id="show<%=i %>an"></span></div>  
...
<a href="javascript:void 0" class="button button-big button-fill button-danger" onclick="bntajax()" id="bnt">提交答卷</a>
  ...
  <a href="lianxi.aspx?wid=<%=wid %>&id=<%=id %>&v=1" class="external" id="next" style=" color:#fff" >再来一次</a>
  </div>
  </div>
</div>
<!--题目 end -->
...
$("#show" + i + "an").html("正确答案："+rean+" "+str);
}


$.toast("你本次练习共答对"+getsum+"道题，请参考正确答案。") 
/////
 $("#next").show();
 $("#bnt").remove();
