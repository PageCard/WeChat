$(function () {
  ...
  $(document).on("pageInit", "#page-infinite-scroll-bottom", function (e, id, page) {
      var loading = false;
      // 每次加载添加多少条目
      var itemsPerLoad = G_pagesize;
      // 最多可加载的条目
      var maxItems = maxcount;
      ...
      $(page).on('infinite', function () {
          ...
          setTimeout(function () {
              ...
              ss++;//分页++
              getajax(ss);
              lastIndex = itemsPerLoad * ss;//计算当前数据总数
              $.refreshScroller();
          }, 1000);
      });
  });
    ...
    ///搜索
  $(document).on("click", ".searchbnt", function (e) {
      var b = $("#search").val();
      var wid = $("#wid").val()
      if (b == "") {
          $.toast("请输入关键词");
          ...
          $.router.load("so.aspx?wid=" + wid + "&sostr=" + b + "");
