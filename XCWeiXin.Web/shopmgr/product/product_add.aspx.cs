...
namespace XCWeiXin.Web.shopmgr.product
{
    public partial class product_add : Web.UI.ManagePage
    {
       ...
       /// <param name="e"></param>
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            string[] remarkArrpx = Request.Form.GetValues("hid_photo_remarkpx");
            if (albumArr != null && albumArr.Length > 0)
            {
                ...
                for (int i = 0; i < albumArr.Length; i++)
                {
                    ...
                    if (imgArr.Length == 3)
                    {
                        ...
                        {
                            ls.Add(new Model.wx_shop_albums { original_path = imgArr[1], thumb_path = imgArr[2], remark = remarkArr[i], px = int.Parse(remarkArrpx[i]) });
                        }
                        else
                        {
                            int px=0;
                            if (remarkArrpx[i] != "")
                                px = int.Parse(remarkArrpx[i]);
                            ls.Add(new Model.wx_shop_albums { original_path = imgArr[1], thumb_path = imgArr[2], px = px });
                        }
                    }
                }
                model.albums = ls;
            ...
            XCWeiXin.BLL.wx_shop_product bll = new XCWeiXin.BLL.wx_shop_product();
