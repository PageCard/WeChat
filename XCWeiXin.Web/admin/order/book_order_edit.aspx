<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="book_order_edit.aspx.cs" Inherits="XCWeiXin.Web.admin.order.book_order_edit" ValidateRequest="false" %>
  ...
  <dl>
    ...
    <dd>
      <table border="0" cellspacing="0" cellpadding="0" class="border-table" width="98%">
        <tr>
          <th width="20%">支付方式</th>
          <td><%=new XCWeiXin.BLL.payment().GetTitle(model.wid.Value,model.payment_id) %></td>
        </tr>
        <tr>
          <th>配送方式</th>
          <td><%=new XCWeiXin.BLL.express().GetTitle(model.express_id) %></td>
        </tr>
        <tr>
          <th>用户留言</th>
          <td><%=model.message %></td>
