...
namespace XCWeiXin.Web.bookmgr.product
{
    public partial class product_add : Web.UI.ManagePage
    {
        ...
        wx_book_catalog_attribute caBll = new wx_book_catalog_attribute();
       ...
       /// <param name="e"></param>
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
           ...
           int id= bll.Add(model);
            AddAdminLog(MXEnums.ActionEnum.Add.ToString(), "添加预订信息，预订信息主键id:" + id); //记录日志

            string frompage = MyCommFun.QueryString("frompage");
            if (frompage == "")
            {
                JscriptMsg("***预订录入成功,请继续录入***！", "product_add.aspx", "Success");
            }
            else
            {
                JscriptMsg("***预订录入成功***！", frompage, "Success");
            }
        }
