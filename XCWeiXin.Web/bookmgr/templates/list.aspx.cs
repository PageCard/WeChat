...
namespace XCWeiXin.Web.bookmgr.templates
{
    public partial class list : Web.UI.ManagePage
    {
        wx_book_templates tBll = new wx_book_templates();

        wx_book_templates_wcode twBll = new wx_book_templates_wcode();
        ...
        private void RptBind(int wid)
        {
            ...
            // 模版数据绑定
            this.rptList2.DataSource = tBll.GetModelList("bookCode='book'");
            this.rptList2.DataBind();

            //绑定当前微帐号选择的模版Id和颜色
            IList<Model.wx_book_templates_wcode> twlist = twBll.GetModelList("wid=" + wid);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (tId != 0)
            {
                ...
                Model.wx_book_templates_wcode tw = new Model.wx_book_templates_wcode();
                bool isAdd = true;
                bool Succ = false;
                if (hidWTId.Value != "0")
                {
