...
function addImage(targetObj, originalSrc, thumbSrc) {
    ...
    + '<input type="hidden" name="hid_photo_remarkpx" value="" />'
    + '<div class="img-box" onclick="setFocusImg(this);">'
    + '<img src="' + thumbSrc + '" bigsrc="' + originalSrc + '" />'
     + '<span class="px"><i>0</i></span>'
    + '<span class="remark"><i>暂无描述...</i></span>'
    + '</div>'
    + '<a href="javascript:;" onclick="setRemarkpx(this);">排序</a>'
...
//设置图片排序
function setRemarkpx(obj) {
    var parentObj = $(obj); //父对象
    var hidRemarkObj = parentObj.prevAll("input[name='hid_photo_remarkpx']").eq(0); //取得隐藏值
    var m = $.dialog({
        lock: true,
        max: false,
        min: false,
        padding: 0,
        title: "图片排序",
        content: '<input id="Imagepx" style="margin:10px 0;font-size:12px;padding:3px;color:#000;border:1px #d2d2d2 solid;vertical-align:middle;width:100px;">',
        button: [{
            name: '　　排　序　　',
            callback: function () {
                var remarkObj = $('#Imagepx', parent.document);
                if (remarkObj.val() == "") {
                    $.dialog.alert('请排序？', function () {
                        remarkObj.focus();
                    }, m);
                    return false;
                }
                hidRemarkObj.val(remarkObj.val());
                parentObj.siblings(".img-box").children(".px").children("i").html(remarkObj.val());
            },
            focus: true
        }]
    });
