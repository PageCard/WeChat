...
function getCmdyCount() {
    //alert("统计购物车数量1" + window.basePath + "/book/bookmgr.ashx?myact=pcount" + "&wid=" + $("#wid").val() + "&openid=" + $("#openid").val());
    var a = $(".icon_number_bubble");
    0 != a.length && utils.ajaxReq({
        url: window.basePath + "/book/bookmgr.ashx?myact=pcount" + "&wid=" + $("#wid").val() + "&openid=" + $("#openid").val(),
        dataType: "json",
        data: {
            t: +new Date
        }
...
var utils = {
    ...
    payDeal: function (a, b) {
        ...
        var returnpage = window.basePath + "/book/userinfo.aspx?wid=" + +$("#hidwid").val() + "&openid=" + $("#openid").val();
        if (a.payType =="1")
        {//货到付款
            location.href = window.basePath + "/book/order_success.aspx?wid=" + $("#hidwid").val() + "&openid=" + $("#openid").val() + "&orderid=" + a.orderid + "&rpage=" + escape(returnpage);
        }
        else{
            //微支付
          //  alert("微支付221");
            location.href = window.basePath + "/api/payment/paypage.aspx?showwxpaytitle=1&paytype=book&wid=" + $("#hidwid").val() + "&openid=" + $("#openid").val() + "&orderid=" + a.orderid + "&rpage=" + escape(returnpage);
        }
...
utils.namespace("mobile.o2ocn.address"), function () {
    ...
    g.init = function () {
        ...
        if (1 > d && (d = 1), d += b.reduce, 1 > d && (d = 1),
            ...
            "1" === c.payType && 500 > a && utils.showBubble("您购买的产品应付总价低于5元，无法使用货到付款功能，请用在线付款下单!"),
            "1" === c.payType)
        {
            var e = b.divPrice(a);
    ...
    g.handleSure = function (a) {
       // alert("确认订单在这里：33,window.pageParam.length：" + window.pageParam);
        ...
        if (!a.hasClass("btn_disabled")) {
            ...
            if ("1" === d && b.payPrice < 500) return utils.showBubble("您购买的产品总价低于5元，无法使用货到付款功能，请用在线付款下单!"), void 0;
            ...
            $.each(window.subParam, function (a, b) {
                ...
                url: window.basePath + "/book/bookmgr.ashx?" + window.baseParam,
                dataType: "json",
                type: "POST",
                data: {
                    myact: "order_save",
                    ...
                    openid: $("#openid").val(),
                    wid:$("#hidwid").val(),
					bookordertime:$("#bookordertime").val(),
					bookorderback:$("#bookorderback").val(),
                    cart: c.cart
                }
            }, function (b) {
    ...
    a.init = function () {
        ...
        a.itemList = $("#item-sec .item"), 0 != a.itemLen && (a.selectAll = $("#choose-all"), a.itemList.each(function () {
            ...
            a.selectItems.number++, a.selectItems.items[e.attr("index")] = e, a.choseNode.push(e), "false" == d && (a.unSupCodList[e.attr("index")] = e, a.unSupCodLen++, a.chosedUnCodLen++), e.on("click", function () {
                var b, c = $(this),
					d = c.attr("index");
                return c.hasClass("icon_checkbox_disabled") ? (utils.showBubble("您选择的产品不支持货到付款，建议您选择在线支付"), void 0) : (b = c.hasClass("icon_checkbox_checked"), b ? (c.removeClass("icon_checkbox_checked"), delete a.selectItems.items[d], a.selectItems.number--) : (c.addClass("icon_checkbox_checked"), a.selectItems.items[d] = c, a.selectItems.number++), a.selectItems.number < a.itemLen ? a.selectAll.removeClass("icon_checkbox_checked") : a.selectAll.addClass("icon_checkbox_checked"), a.callUnCod(b, d), a.changeSel(), void 0)
            })
        ...
        if (!c.hasClass("active")) {
            if (c.hasClass("btn_disabled")) return utils.showBubble("您选择的部分产品不支持货到付款，请选择在线支付"), void 0;
            ...
            }), b.changeSel(), d || (b.selectAll.removeClass("icon_checkbox_checked"), utils.showBubble("已经为您取消了不支持货到付款的产品"))
        }
       ...
       // alert("修改产品数量1:b"+b);
        utils.ajaxReq({
            type: "POST",
            url: window.basePath + "/book/bookmgr.ashx?myact=modifyNum",
            data: {
                ic: d.attr("ic"),
                attr: d.attr("sa"),
                mid: d.attr("mid"),
        ...
        //alert("我在这里e=" + window.basePath + "/book/bookmgr.ashx?myact=remove");
        utils.ajaxReq({
            type: "POST",
            url: window.basePath + "/book/bookmgr.ashx?myact=remove",
            data: i,
            dataType: "json"
        }, function (d) {
    ...
    a.emptyCartIfNone = function (a) {
        a = a ? a : "content", getCmdyCount(), $("#" + a).html("<div class='qb_gap qb_tac qb_pt10' style='padding:70px 0 5px;margin-bottom: 0px;'><img src='/book/templates/default/images/icon_cart_empty.png' width='81' class='qb_mb10'><p class='qb_fs_xl mod_color_comment'>您还有没添加预约</p></div> <div class='qb_tac qb_gap'><a href='" + window.basePath + "/book/index.aspx?wid=" + $("#wid").val() + "&openid=" + $("#openid").val() + "' class='mod_btn'>去逛逛</a></div>")
        ...
        if (!d.hasClass("btn_strong")) return utils.showBubble("请选择产品"), void 0;
       
        if (window.wxInfo && "true" != window.wxInfo.isFocus) return business.focus("购买产品，需要关注该商户，是否立即关注?", $.proxy(a.confirmOrder, this)), void 0;
    ...
    //alert("11");
        ...
        utils.showConfirm({
            describeText: "确定要申请退货吗？如果对产品和服务有任何疑问，建议您先通过微信与我们沟通，我们会热心为您解答。",
            sureText: "申请退货",
            cancelText: "取消",
            sureFn: function () {
        ...
        return;
        return "true" === stockEmpty ? (utils.showBubble("产品库存不足"), void 0) : (0 == a.value ?
        utils.ajaxReq({
    ...
    g.init = function () {
        ...
        if (1 > c && (c = 1), c += b.reduce, 1 > c && (c = 1), a = c + (b.isFreeFee ? 0 : b.curShipFee), "1" == b.curPtype && 500 > a && utils.showBubble("您购买的产品应付总价低于5元，无法使用货到付款功能，请用在线付款下单!"), "1" == b.curPtype) {
            var d = b.divPrice(a);
        ...
        if (!c.hasClass("btn_disabled")) {
            ...
            if (b.promList && 0 == b.promoteId && !b.showed) return utils.showBubble("您的订单可以享受满减优惠，赶快选择优惠方案吧！"), b.showed = !0, !1;
            if (1 == b.curPtype && b.payPrice < 500) return utils.showBubble("您购买的产品总价过低，无法使用货到付款功能，请用在线付款下单!"), !1;
        ...
        var b = window.pageParam,
			...
			d = [],
			e = this;
        if (a ? "1" !== b.comeFrom ? c.splice(0, 1) : c.length > 0 && (c[0].desc = "收藏产品减免运费", c[0].guide = !0) : (e.reduce = 0, e.isFreeFee = !1, e.promoteId = 0), c.length <= 0) return e.countPrice(), void 0;
    ...
    a.init = function () {
        ...
        if (this.buyNode.hasClass("btn_strong")) {
            ...
            if (window.wxInfo && "true" !== window.wxInfo.isFocus) return business.focus("购买产品，需要关注该商户，是否立即关注?", $.proxy(a.handleBuy, this)), void 0;
       ...
       // alert("window.basePath=" + window.basePath + "this.buyNum" + this.buyNum + "ic=" + $("#ic").val() + " attr=" + this.buyProperty + " mid=" + window.availSku[this.buyProperty].pid + " bc=" + this.buyNum + " openid=" + $("#openid").val() + " totprice=" + $("#totprice").val());//我添加的
        utils.ajaxReq({
            url: window.basePath + "/book/bookmgr.ashx",
            dataType: "json",
            type: "POST",
            data: b
        }, function (b) {
            ...
            b.errCode ? utils.showConfirm({
                ...
                describeText: b.msgType ? b.errMsg : "加入预约失败，请稍候再试",
                sureText: "重试",
                cancelText: "取消",
                sureFn: function () {
                ...
                sureFn: function () {
                    ...
                    location.href = window.basePath + "/book/cart.aspx?wid=" + $("#wid").val() + "&openid=" + $("#openid").val()
                }
            })
        })
        ...
        function c() {
            ...
            if (g = g ? JSON.parse(g) : "", a = g ? g[h] : "", a && (b = a.time, 1728e5 >= i - b ? c = a.imgList : (delete g[h], e.setValue(g, !0))), c) d(c);
                ...
                utils.ajaxReq({
                    ...
                    if (f.html("产品相册").removeClass("clicking"), clearInterval(k), a.errCode) utils.showAjaxErr(a, "拉取详情图片失败");
                    else {
                        c = a.data, d(c);
                        var b = {};
                        b[h] = {
                    ...
                    clearInterval(k), f.removeClass("clicking").html("产品相册"), utils.showBubble("拉取详情图片失败")
                })
            }
        }
        function d(a) {
    ...
    a.init = function () {
        ...
        for (var b, c, d, e, f, g, h = [], i = 0, j = a.length; j > i; i++) {
            ...
            for (var k = 0, l = c.length; l > k; k++) {
                ...
                d.push(utils.strReplace(this.itemListTpl, {
                    ...
                    itemName: g.name,
                    attr: g.mid ? '<p class="qb_fs_s ui_color_weak">产品编号：' + g.sn + "</p>" : "",
      ...
      var  order_id=a.getAttribute("dc");
        ...
        var returnpage = window.basePath + "/book/userinfo.aspx?wid=" + +$("#hidwid").val() + "&openid=" + $("#openid").val();
        //微支付
       location.href = window.basePath + "/api/payment/paypage.aspx?showwxpaytitle=1&paytype=book&wid=" + $("#hidwid").val() + "&openid=" + $("#openid").val() + "&orderid=" + order_id + "&rpage=" + escape(returnpage);
