    ...
    k = {
        ...
        getBrandinit: function() {
            ...
            b = {
                ...
                insertObj: a.prev(),
                total: parseInt(a.prev().children().filter(".book_list").length, 10),
...
var VsmCart = function() {
            ...
            function k() {
                ...
                if (f.html(""), sec = i.nowTime < i.endTime ? i.endTime - i.nowTime: 0, null != h.showMsg && (c.msg = h.showMsg), h.showMin && (c.min = b.addZero(h.minDigit, Math.floor(sec / 60 % 60))), c.sec = b.addZero(h.minDigit, Math.floor(sec % 60)), !(i.endTime > i.nowTime)) {
                    ...
                    case "remain":
                        ...
                        !$('[data-getvercode="code"]').length && g("您的预订已超时被清空，继续抢购吧", {
                            title: "唯品会友情提醒您",
                            buttons: !0
                        });
                        break;
    ...
    d = {
        ...
        tiggerDelGoods: function(a) {
            ...
            c = "确定从购物车中删除该预订吗？";
            b > 0 && (c = "删除或修改预订后，可能会导致已使用的红包，不满足条件而被自动取消使用，是否继续操作？"),
            0 > a && !isNaN(a) ? "": g(c, {
                title: "删除预订",
                overClose: !1,
                buttons: !0,
        ...
        ProductGetData: function(b) {
            ...
            function(c) {
                ...
                function(a, b) {
                    ...
                    $_tableStr += "</tr>"
                }), $("#J_tableBody").prepend($_tableStr), b[c].tips.length > 0 && $("#J_size_tips").append(b[c].tips).removeClass("hidden")) : b[c].content.length > 0 && "html" == b[c].type ? ($("#wap_Size_data").html(b[c].content), $("#wap_Size_data").children().find("table").removeClass("table").addClass("wap_size_tab")) : ("error" == b[c].type || 0 == b[c].content) && $("#wap_Size_data").html('<p class="font14">此预订没有尺码</>');
